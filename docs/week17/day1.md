...
手册任务：画出 Redis（短期）+ PG（长期）+ 向量库（语义/情景）三层记忆架构图。拆成 5 步，全程约 20 分钟。用 [Excalidraw](https://excalidraw.com) 或纸笔即可——下面每一步先把结构用文字列清楚，你在画布上照着摆。

**第 1 步：起画布。** 在本周练习目录新建 `memory-architecture.md` 记录结构草图，画布用 Excalidraw（或纸笔）。下面每一步的结构都记进去，画完它就是当日产出。

**第 2 步：立起三个存储框。** 先不管箭头，把三层按「离 Agent 的远近」从上往下摆：短期最近在最上，长期居中，向量库垫底。

- **短期记忆 · Redis**：当前会话最近 N 轮
- **长期记忆 · PG**：用户档案——偏好 / 事实 / 约束
- **情景 + 语义 · 向量库**：历史对话事件 + RAG 知识库

**第 3 步：加 Agent 和读写箭头。** Agent 画在最顶上，对每层各画一条双向标注的箭头，标签写清动作和时机，时机全部来自矩阵表，不要发明新动作：

- **Agent 运行时 ⇄ 短期记忆（Redis）**：每轮追加写入 / 组 prompt 时读最近上下文
- **Agent 运行时 ⇄ 长期记忆（PG）**：提炼事实后写入 / 会话开始注入
- **Agent 运行时 ⇄ 情景 + 语义（向量库）**：会话结束归档 / 相似度检索

六组读写动作画完后，拿矩阵表逐行核对：每个「何时写」「何时读」在图上都能找到对应的箭头吗？找不到就补，多余的就删。

**第 4 步：标注生命周期，得到终稿。** 给每个存储框补上第二行——Redis 加「TTL，会话结束即过期」，PG 加「永久，可更新覆盖」，向量库加「长期，只增可标废弃」。标生命周期不是装饰，它回答「这层数据什么时候死」，是本周写代码时 TTL 参数和清理策略的直接依据。终稿的结构：

- **短期记忆 · Redis**——当前会话最近 N 轮｜生命周期：TTL，会话结束即过期
- **长期记忆 · PG**——用户档案：偏好 / 事实 / 约束｜生命周期：永久，可更新覆盖
- **情景 + 语义 · 向量库**——历史对话事件 + RAG 知识库｜生命周期：长期，只增可标废弃
- **读写关系**（Agent 居顶，三条双向箭头）：每轮追加 / 组 prompt 时读最近上下文；提炼后写 / 会话开始注入；结束时归档 / 相似度检索
...
::: tip 画图工具
Excalidraw 画三色横条堆叠、Agent 居顶、实线箭头表示读、虚线表示写，导出 PNG 存档当本周的施工图；上面几组结构列表留在笔记里，代码一动就对照更新。
:::
...
今天的产出 `memory-architecture.md`（结构清单 + 手绘图）留好，这是本周的施工蓝图：接下来几天会把它一层层变成真代码，Redis 对话窗口、PG 用户档案、向量库情景检索依次落地，进度对照[本周计划](/week17/)推进。从[第 1 周](/week01/)的 `createResponse<T>` 走到今天的记忆架构，零件早已齐备，这周正式拼装。
