...
| **检查发生在** | 运行时（逻辑你自己写） | 运行时（失败必须抛错） | **编译期** |
...
| 写法 | 检查时机 | 失败行为 | 改变类型吗 | 一句话定位 |
| --- | --- | --- | --- | --- |
| `typeof` / `in` / `instanceof` | 编译期推断 + 运行时真判断 | 走 else | 分支内窄化 | 内建工具，优先用 |
| 判别联合 `kind` | 编译期 | 不适用 | switch 分支窄化 | 数据设计的窄化 |
| `is` | 运行时（自定义逻辑） | 返回 false | true 分支窄化 | 外部数据的守门员 |
| `asserts` | 运行时（自定义逻辑） | 抛错中断 | 调用后窄化 | 不可能的分支就崩 |
| `satisfies` | 编译期 | 编译报错 | **不改**，保留推导 | 字面量的质检章 |
| `: 注解` | 编译期 | 编译报错 | 抬宽到声明类型 | 声明契约，丢细节 |
| `as` | 无（纯断言） | 不失败（骗编译器） | 强制视为目标类型 | 最后手段 |
...
::: details 查看答案
is 和 asserts 都能处理：is 返回 boolean、true 分支窄化、false 走兜底逻辑；asserts 抛错式快崩、调用点后窄化。satisfies 什么都做不了——它只在编译期检查字面量的兼容性，对运行时才确定值的 unknown 无从下手。一句话：前两者是运行时守门员（宽容 vs 严格），satisfies 是字面量的编译期质检章。
:::
